...
// Adapted by Andres Bell Navas (22.03.17)
//==============================================================================
...
class IndexValuePair {
 public:
  IndexValuePair() { }
  IndexValuePair(uint i, float v) : first(i), second(v) { }
  uint first;
  float second;
};
...
/** Implements a simple sparse vector based on stl vector.
	...
	@extension Andrés Bell Navas (abn@gti.ssr.upm.es)
*/
class simple_sparse_vector {
  ...
  */
  ...
  // Add the value of the feature number specified with key into the vector
  void addElement(uint key, float value);

  //Get the non-zero values in each sample of a determined element (between zero and total_dimension-1). Element in this case is the iterator
  void getCol(std::vector<simple_sparse_vector> &samples, uint numElement);

  //Get the mean and standard deviation of the simple_sparse_vector data
  void meanStdDev(uint num_samples, float &mean, float &std);
