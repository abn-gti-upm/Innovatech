...
void LearnReturnLastAdapted(// Input variables
	std::vector<simple_sparse_vector>& Dataset,
	std::vector<int>& Labels,
	uint dimension,
	//std::vector<simple_sparse_vector>& testDataset,
	...
	double lambda = 1.0; //default 0.001
	int max_iter = 10; //default 100/lambda
	int exam_per_iter = 1; //default 1 (size of block for stochastic gradient)
	...
	//long startTime = get_runtime(); //optional
	...
	std::vector <WeightVector> hola(5);

	std::vector<WeightVector> prueba;
	prueba.resize(7);
	...
	for (int i = 0; i < max_iter; ++i) {
		...
		for (int j = 0; j < exam_per_iter; ++j) {
			...
			// choose random example
			//srand(time(NULL)); //introduce seed?
			uint r = ((int)rand()) % num_examples;
...
//	}
	...
	if (output_filename != "noModelFile") {
		std::ofstream model_file(output_filename.c_str());
		if (!model_file.good()) {
